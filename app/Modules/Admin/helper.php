...
function ForgotPasswordEmail($name,$email,$token,$role_id)
    ...
    * {
    ...
    <a href='$baseurl' target='_blank'><p style='font-size:13px;line-height:5px'>$baseurl</p></a>
    </div>
   ...
   function user_appwallet_get($id)
   {
      $row=DB::table('tbl_app_user_wallet')->select('app_user_wallet_amount','app_user_wallet_id')->where(['user_uniqueid'=>$id])->get();

     return $row;
   }

   function user_order_count($id)
   {
     $data=DB::table('tbl_app_order')
     ->select('*')->where(['user_uniqueid'=>$id])->count();
     return $data;
   }

   function user_reportproblem_count($id,$order_id)
   {
     $data=DB::table('tbl_app_order_report_problem')
     ->select('*')->where(['user_uniqueid'=>$id,'app_order_id'=>$order_id])->count();
     return $data;
   }
