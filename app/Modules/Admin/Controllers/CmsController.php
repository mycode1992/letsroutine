...
class CmsController extends Controller
{
    ...
    public function add_faq($id=null)  
    {
        ...
        if(isset($id) && $id!=null)
        {
       $data = DB::table('tbl_lr_cms')->where('lr_cms_name','Faq')->select('lr_cms_id','lr_cms_title','lr_cms_description','status')->where('lr_cms_id',$id)->get();
       return view("Admin::cms.save_faq",compact('data'));
        }
        else{
    ...
    public function submit_faq(Request $request) // start of submit_faq function
    {
        if($request->ismethod('post'))
        {
         ...
         }//update id cehck if close  
         else if($request->updateid!='') 
         {
         //DB::enableQueryLog();   
         $faq_title = $request->faq_title;
         $faq = $request->faq;
         $sql_insert = DB::table('tbl_lr_cms')->where('lr_cms_id',$request->updateid)->update([
             'lr_cms_name' => 'Faq',
             'lr_cms_title' => $faq_title,
             'lr_cms_description' => $faq,
         ]);
