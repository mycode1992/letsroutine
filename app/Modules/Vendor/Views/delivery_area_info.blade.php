@include('Vendor::include.header')
	...
	<div>
		...
		<?php
			$delverytime = DB::table('tbl_lr_vendor_deleverytime')->where('vendor_areaid',$area_val->id)->select('id','from','to')->get();
		?>
		...
		<td class="delivery-timer">
			<div>
				<a href="javascript:void(0)" onclick="return updatetimeModal({{$area_val->id}})" class="add-time-btn"><span class="glyphicon glyphicon-time"></span>Add Time</a>
			</div>
		</td>
		</tr>
		<?php } ?>
