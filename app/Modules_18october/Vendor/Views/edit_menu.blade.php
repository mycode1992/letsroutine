...
{  
$updateid = $data[0]->lr_unique_id;
$vendor_menu_name = $data[0]->vendor_menu_name;
$butttext = 'Update';
$page_name = 'Edit Menu';
}
@endphp
...
<div class="col-md-9">
<div class="rightBox">
<h3 class="formTitle">Edit Menu</h3>
<form id="formData" onsubmit="return save_menu();" >
<input type="hidden" name="updateid" id="updateid" value="{{$order_unique_id}}"/>
<div class="col-md-12">
<div class="form-group">
<label for="">Menu name<span>*</span></label>
<input type="text" name="menu_name" id="menu_name" value="{{$vendor_menu_name}}" placeholder="Enter menu name"  class="form-control">
...
<select name="menu_category" id="menu_category" onchange="return showmeal_menu1()" data-placeholder="Select Menu" multiple class="form-control select2">
@foreach($menu_list AS $val)
<option value="{{$val->lr_category_id}}" <?php if(isset($data_menu_cat)){ foreach($data_menu_cat AS $val1){ if($val->lr_category_id==$val1->vendor_menu_category){echo 'selected';} } } ?> >{{$val->lr_category_name}}</option>
@endforeach
...
@foreach($menu_list AS $val)
			<div id="{{$val->lr_category_id}}" class="row fornone" style="display:none" >
			<div class="col-md-6">
				<div class="form-group">
					<label for="">{{$val->lr_category_name}}<span>*</span></label>
					...
					<option value="{{$value->lr_meal_id}}" <?php if(isset($selected_mealid)){foreach($selected_mealid AS $selmeal_val){if($selmeal_val->vendor_menu_category==$val->lr_category_id && $selmeal_val->vendor_menu_category_meal==$value->lr_meal_id){echo 'selected';}}} ?> >{{$value->lr_meal_name}} </option>
							@endforeach
					</select>
			</div>
		</div>
...
<?php
$jsonData = json_encode($menu_list);
?>
...
<script>
		function showmeal_menu1(id)
		{
			...
			jsonData.forEach(function(item,index,array){
				let itemId = item.lr_category_id;
    ...
    function documentready()
	 {
		...
		form.append('order_unique_id', order_unique_id); 
		

		$.ajax({    
		headers: {'X-CSRF-Token':'{{ csrf_token() }}'},
		type: 'POST',
		url: "{{url('/vendor/edit_showmeal_menu')}}",
		data:form,
		contentType: false,
		processData: false,
		...
		for(var j=0; j < count_meal_list; j++){
            for(var k=1; k < Object.keys(response.menu_meal_id).length; k++)
            {
                if(meal_list[j].lr_meal_id==response.menu_meal_id[k][j])
                {
                    string += '<option value="'+meal_list[j].lr_meal_id+'" '+ selected +' >'+meal_list[j].lr_meal_name+'</option>';
                }
                else
                {
