...
class VendorController extends Controller
{
    ...
    {
        return view("Vendor::current_order");
    }
    ...
    {  
        ...
        $data = DB::table('tbl_lr_vendor_menu')->where('admin_status','!=','2')->select('lr_unique_id','vendor_menu_name')->distinct('lr_unique_id')->orderBy('vendor_menu_id','desc')->get();
    ...
    {  
        ...
        $data = DB::table('tbl_lr_meal')->where('admin_status','!=','2')->select('lr_meal_id','lr_user_userid','lr_meal_name','lr_meal_calories','lr_meal_fat','lr_meal_carb','lr_meal_protein','lr_meal_description')->orderBy('lr_meal_id','desc')->get();
    ...
    public function add_plan($id=null)
    {
        ...
        if($id!='')
        {
            $data = DB::table('tbl_lr_vendor_plan')->where('vendor_plan_id',$id)->select('vendor_plan_id','vendor_plan_name','vendor_plan_meals','vendor_plan_snacks','vendor_plan_macros','macros_yes_min_carb','macros_yes_max_carb','macros_yes_min_protein','macros_yes_max_protein','macros_yes_plan','macros_no_max_carb','macros_no_max_protein','vendor_plan_gendor','vendor_plan_pause','lr_plancategory_id','lr_plan_subcat_id','vendor_plan_duration','vendor_plan_offdays','vendor_plan_menu_type','custom_menu_vendor_menu_id','vendor_plan_type','plantype_weekly','plantype_weekly_packagefeature','cost_plan_package_feature','plan_cost')->get();
            
            return view("Vendor::add_plan",compact('category_plan','menu_list','data','package_feature','menu_list_m','meal_list_m'));
        }
        else
        {
            return view("Vendor::add_plan",compact('category_plan','menu_list','package_feature','menu_list_m','meal_list_m'));
        }
   ...
   public function plan_calender($plan_id)
   {
    ...
    if($plan_id!='')
    {
        $data = DB::table('tbl_lr_vendor_plan')->select('vendor_plan_menu_type','vendor_plan_meals','vendor_plan_snacks')->where('vendor_plan_id',$plan_id)->get();
        ...
        $isvendor_exists = DB::table('tbl_lr_vendor_plandetail')->where('lr_user_id',$sessionuserid)->where('vendor_plan_id',$plan_id)->min('vendor_plandetail_date');

        if($isvendor_exists!='')
        {
            $date_start = $isvendor_exists;
   ...
   public function plan_changemeal_snacks(Request $request)
        ...
        public function save_plan_detail(Request $request)
        {
            if($request->isMethod('post'))
            {
              ...
              foreach($mealcheckbox AS $key => $value)
              {
                ...
                {
                    $sql = DB::table('tbl_lr_vendor_plandetail')->insert([
                        'lr_user_id' => $sessionuserid,
                        'lr_unique_id' => $lr_unique_id,
                        'vendor_plan_id' => $planid,
        ...
        public function edit_plan_detail(Request $request)
        {
            if($request->isMethod('post'))
            {
                ...
                $planid = $request->planid; 
                $isdataavailable = DB::table('tbl_lr_vendor_plandetail')->where('vendor_plandetail_date',$date)->where('vendor_plan_id',$planid)->count();
                if($isdataavailable>0)
                {
                  $menuname_meal = DB::table('tbl_lr_vendor_plandetail')
                 ...
                 $menuname_snacks = DB::table('tbl_lr_vendor_plandetail')
                                    ->where('vendor_plandetail_date',$date)
                                    ->where('vendor_plan_id',$planid)
                                    ->where('lr_category_type','SNACKS')
                                    ->select('vendor_menu_id')->distinct('vendor_menu_id')->get();
             
                $menu_category_meal = DB::table('tbl_lr_vendor_plandetail as t1')   
                ...
                $menu_category_snacks = DB::table('tbl_lr_vendor_plandetail as t1')   
                ->join('tbl_lr_menu_category as t2','t1.lr_menucategory_id','t2.lr_category_id')
                ->where('vendor_plandetail_date',$date)
                ...
                $meal_id_snacks = DB::table('tbl_lr_vendor_plandetail as t1')   
        ...
        public function get_meal_edit(Request $request)
        {
            if($request->isMethod('post'))
            {
                ...
                $meal_list = DB::table('tbl_lr_vendor_plandetail as t1')
                            ->join('tbl_lr_meal as t2','t1.lr_meal_id','t2.lr_meal_id')
        ...
        public function update_plan_detail(Request $request)
        {
            if($request->isMethod('post'))
            {
              ...
              $sql_del = DB::table('tbl_lr_vendor_plandetail')->where('vendor_plandetail_date',$date)->where('vendor_plan_id',$planid)->delete();
